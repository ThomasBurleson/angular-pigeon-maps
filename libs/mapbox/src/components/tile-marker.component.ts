...
export class TileMarkerComponent {
  ...
  @Input()  set selected(value:boolean) { this._isSelected = value;  this.select.emit(this); }
            get selected():boolean      { return this._isSelected; }
  
  @Output() select : EventEmitter<TileMarkerComponent> = new EventEmitter<TileMarkerComponent>(); 
  ...
  get markerUrl():string {
    const url = this.selected  ? 
                (isRetina()    ? 'pin-hover@2x.png' : 'pin-hover.png') :
                (isRetina()    ? 'pin@2x.png'       : 'pin.png');
