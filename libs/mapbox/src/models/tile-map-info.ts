...
export class TileMapCoordinates {
  ...
  centerAndZoom(
    ...
    const offset = pixelDelta
      ? pixelDelta.scale(1 / 256.0 / scale)
      : new Point(0, 0);

    (this.center = new Point(converted.x - offset.x, converted.y - offset.y)),
      (this.min = new Point(
        Math.floor(this.center.x - halfWidth),
        Math.floor(this.center.y - halfHeight)
      )),
      (this.max = new Point(
        Math.floor(this.center.x + halfWidth),
        Math.floor(this.center.y + halfHeight)
  ...
  toLatLong(target: Point): LatLongPoint {
    ...
    const delta = new Point(
      target.x - this.scaling.width / 2,
      target.y - this.scaling.height / 2
    )
